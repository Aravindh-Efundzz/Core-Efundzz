...
//    }
//    @GetMapping("/application/{id}/getTaskKeys/{taskKey}")
//    public ResponseEntity<?> getTaskKeys(@PathVariable String id, @PathVariable String taskKey){
//        return ResponseEntity.ok(loanService.getTaskKeys(id, taskKey));
//    }
...
//        return ResponseEntity.ok(Optional.ofNullable(s3Service.getPresignedUrl(operation, fileName)).map(e -> e.toString()).orElse(""));
//    }
//
...
//    public Object getPAN(@PathVariable String applicationId) {
//            try {
//                StepData stepData = stepDataRepository.findTopByApplicationIdAndStepName(applicationId, "LoanDetails");
//                if (stepData.getData().get("pan") == null) {
