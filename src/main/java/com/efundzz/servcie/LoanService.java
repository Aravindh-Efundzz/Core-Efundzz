        ...
        try {
            ...
            if (!Objects.requireNonNull(userLoans).isEmpty()) {
                ...
                if (savedLoanResponse.getStatusCode().is2xxSuccessful()) {
                    ...
                    String generatedId = savedLoan.getId();
    ...
    public ResponseEntity<?> completeTask(String id, String taskKey, Map<String, Object> variables) {
        Loan loan = dataBaseFeignClient.getLoanById(id).getBody();
            ...
            ProcessInstance updatedProcessInstance = runtimeService.createProcessInstanceQuery()
                    .processInstanceId(retrievedProcessInstance.getId()).singleResult();
...
//        }
//
//        List<PartnerEntity> partnerEntities = partnerEntityRepository.findAll();
//
//        List<String> finalOffers = offers;
//        finalOffers.replaceAll(String::toUpperCase);
//        List<PartnerEntity> filteredOffers = partnerEntities.stream()
...
//        String userId = authentication.getName();
//        List<Loan> loans = loanRepository.findByUserIdAndBrand(userId, brand);
//        long submittedCount = loans.stream()
...
//
//        for (Loan loan : loans) {
//            List<StepData> stepDataList = stepDataRepository.findByApplicationId(loan.getId());
...
//                .singleResult()
//                .getId();
//        StepData existingStepData = stepDataRepository.findTopByApplicationIdAndStepName(id, taskKey);
...
//            stepDataRepository.save(existingStepData);
...
//        String userId = authentication.getName();
//        Loan loan = loanRepository.findTopByIdAndUserId(id, userId);
//
//        if(loan == null){
...
//            FlowNode sourceElement = (FlowNode) incomingFLows.iterator().next().getSource();
...
//                .collect(Collectors.toList());
//    }
//    public ResponseEntity<TaskKeysDTO> getTaskKeys(String id, String taskKey) {
//        ProcessInstance retreivedProcessInstance = runtimeService.createProcessInstanceQuery()
//                .processInstanceBusinessKey(id)
//                .singleResult();
...
//        String userId = authentication.getName();
//        List<StepData> stepDataList = stepDataRepository.findByApplicationId(id);
//
...
//
//        if(loan == null){
//            return new ResponseEntity<>(new HashMap<String, Object>(), HttpStatus.UNAUTHORIZED);
//        }
//
//
//        HistoricProcessInstance historicProcessInstance = historyService.createHistoricProcessInstanceQuery()
...
//        return (FlowNode) bpmnModelInstance.getModelElementById(incomingSequenceFlow.getSource().getId());
//    }
//
...
//
//        if(loan == null){
//            return new ResponseEntity<>(new HashMap<String, StepData>(), HttpStatus.UNAUTHORIZED);
//        }
//
...
//
//        Map<String, StepData> groupedStepData = stepDataList.stream()
...
//            loanRepository.findByUserId(uid).forEach(loan -> {
//                if (!loan.getStatus().equalsIgnoreCase("COMPLETED")) {
...
//
//        referenceDataRepository.findByRefKey1AndRefKey2(loanType.getId(), RefKey.DOCUMENT_MANDATORY.name()).forEach(ref -> {
//            switch (ref.getKey()) {
//                case SALARY1:
